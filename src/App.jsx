...
const TRANSLATIONS = {
    fr: {
        subtitle: "Journal de terrain(s)", searchPlaceholder: "Rechercher...", clear: "Effacer", newSignal: "Nouveau Signal", editSignal: "Éditer le Signal", 
        idPlace: "Identifiant / Lieu", notes: "Notes de terrain...", mediaUrl: "URL Média", dateTimeStr: "Date et Heure", save: "Enregistrer", context: "Contexte & Tags", 
        lat: "Latitude", lng: "Longitude", types: { text: "Texte", photo: "Photo", video: "Vidéo", audio: "Audio" }, 
        selectHint: "Sélectionnez un ou plusieurs thèmes", timeline: "Chronologie",
        aboutProjectBtn: "À Propos", aboutAuthorBtn: "Misia Forlen", aboutTitle: "À Propos", authorTitle: "Misia Forlen",
        aboutProjectTitle: "Le Projet: Habiter a Zone", aboutProjectDesc: "Une exploration de recherche-création documentant les conditions de vie des travailleurs mobiles.",
        aboutAuthorTitle: "L'Auteure: Misia Forlen", aboutAuthorDesc: "Architecte et doctorante au sein du programme RADIAN.",
        mapType: "Carte", mapStyleDark: "Sombre", mapStyleLight: "Clair", mapStyleSat: "Sat", directionView: "Angle de Vue",
        login: "Connexion Système", email: "E-mail", password: "Mot de passe", enter: "Entrer", edit: "Éditer", duplicate: "Dupliquer", delete: "Supprimer", deleteConfirm: "Êtes-vous sûr ?",
        manageTags: "Gérer les Tags", newTag: "Nouveau Tag", editTag: "Éditer le Tag", tagName: "Nom du tag", tagColor: "Couleur", addTag: "Ajouter"
    },
    en: {
        subtitle: "Mapping System", searchPlaceholder: "Search...", clear: "Clear", newSignal: "New Signal", editSignal: "Edit Signal", 
        idPlace: "ID / Location", notes: "Field notes...", mediaUrl: "Media URL", dateTimeStr: "Date & Time", save: "Save", context: "Context & Tags", 
        lat: "Lat", lng: "Lng", types: { text: "Text", photo: "Photo", video: "Video", audio: "Audio" }, 
        selectHint: "Select one or more themes", timeline: "Timeline",
        aboutProjectBtn: "About", aboutAuthorBtn: "Misia Forlen", aboutTitle: "About", authorTitle: "Misia Forlen",
        aboutProjectTitle: "The Project: Habiter a Zone", aboutProjectDesc: "A research-creation exploration documenting mobile workers.",
        aboutAuthorTitle: "The Author: Misia Forlen", aboutAuthorDesc: "Architect and PhD candidate in the RADIAN program.",
        mapType: "Map", mapStyleDark: "Dark", mapStyleLight: "Light", mapStyleSat: "Sat", directionView: "View Angle",
        login: "System Login", email: "Email", password: "Password", enter: "Enter", edit: "Edit", duplicate: "Duplicate", delete: "Delete", deleteConfirm: "Are you sure?",
...
const App = () => {
    ...
    return (
        <div className="relative w-full h-screen font-mono text-gray-200 bg-black">
            ...
            <div className="absolute top-0 right-0 w-[400px] h-full p-4 z-[900] pointer-events-none overflow-y-auto">
                <div className="pointer-events-auto flex flex-col gap-3">
                    {timelineMemories.map(mem => (
                        <div key={mem.id} ref={el => timelineRefs.current[mem.id] = el} onClick={() => setSelectedMemory(mem)} className={`p-4 bg-black/90 border transition-all ${selectedMemory?.id === mem.id ? 'border-hlzPurple shadow-lg' : 'border-gray-800'} cursor-pointer`}>
                            ...
                            {selectedMemory?.id === mem.id && (
                                <div className="mt-2 text-xs text-gray-400 space-y-3 fade-in">
                                    {mem.type === 'photo' && mem.content && <img src={mem.content} onClick={() => setFullScreenItem({type: 'photo', src: mem.content})} className="w-full grayscale hover:grayscale-0" alt="terrain"/>}
                                    {mem.type === 'video' && mem.content && <div className="relative" onClick={() => setFullScreenItem({type: 'video', src: mem.content})}><video src={mem.content} className="w-full opacity-60" /><div className="absolute inset-0 flex items-center justify-center text-hlzPurple">▶</div></div>}
                                    <p className="leading-relaxed border-l border-hlzPurple pl-2">{mem.description}</p>
                                    <div className="flex flex-wrap gap-1">
                                        {mem.tags?.map(tg => <span key={tg} style={{color: getDerivedTagColor(tg)}} className="text-[9px] border border-gray-800 px-1 uppercase">#{tg}</span>)}
                                    </div>
                                    {session && (
                                        <div className="flex gap-4 border-t border-gray-900 pt-3">
                                            <button onClick={() => deleteMemory(mem.id)} className="text-red-500 flex items-center gap-1 hover:text-white transition-colors"><Trash2 size={12}/> DELETE</button>
                                            <button onClick={() => {setEditingId(mem.id); setNewMemory({...mem, datetime: mem.date, tags: mem.tags.join(', ')}); setIsAdding(true);}} className="text-blue-400 flex items-center gap-1 hover:text-white"><Edit size={12}/> EDIT</button>
                                        </div>
                                    )}
                                </div>
                            )}
            ...
            {/* MODAL: NOVO PONTO (DATA E HORA INCLUÍDOS) */}
            {isAdding && session && (
                <div className="fixed inset-0 z-[2500] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="industrial-panel p-6 w-full max-w-md bg-black border border-hlzPurple shadow-2xl">
                        <div className="flex justify-between mb-4 border-b border-gray-800 pb-2">
                            <h2 className="text-xl font-bold uppercase">{editingId ? t.editSignal : t.newSignal}</h2>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-2">
                                {['text', 'photo', 'video', 'audio'].map(tp => (
                                    <button key={tp} onClick={() => setNewMemory({...newMemory, type: tp})} className={`flex-1 py-1 text-[10px] border ${newMemory.type === tp ? 'bg-hlzPurple text-black font-bold' : 'border-gray-700 text-gray-500'}`}>{t.types[tp]}</button>
                                ))}
                            </div>
                            ...
                            <input type="text" placeholder="Tags (ex: art, habitat)" className="w-full industrial-input p-2 text-xs" value={newMemory.tags} onChange={e => setNewMemory({...newMemory, tags: e.target.value})} />
                            
                            <button onClick={handleSaveMemory} className="w-full bg-hlzPurple text-black font-bold py-3 uppercase hover:bg-white transition-colors">Sauvegarder le Point</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {aboutTab && (
                <div className="fixed inset-0 z-[3000] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <div className="industrial-panel p-8 w-full max-w-2xl bg-black border border-hlzPurple overflow-y-auto">
                        <div className="flex justify-between border-b border-gray-800 pb-4 mb-6">
                            <h2 className="text-3xl font-bold uppercase">{aboutTab === 'project' ? t.aboutTitle : t.authorTitle}</h2>
                            <button onClick={() => setAboutTab(null)}><X size={24}/></button>
                        </div>
                        <p className="text-gray-400 leading-relaxed text-justify">
                            {aboutTab === 'project' ? t.aboutProjectDesc : t.aboutAuthorDesc}
                        </p>
                    </div>
                </div>
            )}
            ...
            {showLogin && (
                <div className="fixed inset-0 z-[3000] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <form onSubmit={handleLogin} className="industrial-panel p-6 w-full max-w-sm bg-black border border-hlzPurple">
                        <div className="flex justify-between mb-6 border-b border-gray-800 pb-2"><h2 className="text-xl font-bold uppercase"><Lock size={16}/> LOGIN</h2><button onClick={() => setShowLogin(false)}><X/></button></div>
            ...
            {showTagManager && session && (
                <div className="fixed inset-0 z-[3000] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <div className="industrial-panel p-6 w-full max-w-md bg-black border border-hlzPurple max-h-[80vh] overflow-y-auto">
                        <div className="flex justify-between mb-6 border-b border-gray-800 pb-2"><h2 className="text-xl font-bold uppercase"><Tag size={16}/> GESTION TAGS</h2><button onClick={() => setShowTagManager(false)}><X/></button></div>
                        <div className="space-y-2 mb-6">
                            {dbTags.map(tag => (
                                <div key={tag.id} className="flex justify-between p-2 border border-gray-800">
                                    <span style={{color: tag.color}} className="uppercase text-sm font-bold">#{tag.name}</span>
                                    <div className="flex gap-2 text-gray-500">
                                        ...
                                        <button onClick={async () => { if(window.confirm(t.deleteConfirm)) await supabase.from('tags').delete().eq('id', tag.id); window.location.reload(); }}><Trash2 size={14}/></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                        <div className="border-t border-gray-800 pt-4 space-y-3">
                            <input type="text" placeholder="Tag Name" className="w-full industrial-input p-2" value={newTagName} onChange={e => setNewTagName(e.target.value)} />
                            <div className="flex gap-2">
                                <input type="color" className="h-10 w-12 bg-transparent" value={newTagColor} onChange={e => setNewTagColor(e.target.value)} />
            ...
            {fullScreenItem && (
                <div className="fixed inset-0 z-[4000] bg-black/95 flex items-center justify-center p-4" onClick={() => setFullScreenItem(null)}>
                    ...
                    {fullScreenItem.type === 'photo' ? <img src={fullScreenItem.src} className="max-w-full max-h-full" alt="zoom"/> : <video src={fullScreenItem.src} controls autoPlay className="max-w-full max-h-full" />}
                </div>
            )}
            
            {/* RODAPÉ ACESSO RÁPIDO */}
            <div className="fixed bottom-4 left-4 z-[2000] flex gap-2">
                <button onClick={() => setAboutTab('project')} className="text-[10px] bg-black/80 p-1 border border-gray-800 hover:text-hlzPurple tracking-widest uppercase">{t.aboutProjectBtn}</button>
                <button onClick={() => setAboutTab('author')} className="text-[10px] bg-black/80 p-1 border border-gray-800 hover:text-hlzPurple tracking-widest uppercase">{t.authorTitle}</button>
                {session && <button onClick={() => supabase.auth.signOut()} className="text-[10px] text-red-500 bg-black/80 p-1 border border-gray-800 uppercase">LOGOUT</button>}
            </div>
        </div>
    );
};
